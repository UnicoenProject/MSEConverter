        ...
        public void Generate(string dirPath, TextWriter writer) {
            ...
            foreach (var filePath in filePaths) {
                ...
                _visitor.DefaultNamespace = UnifiedNamespaceDefinition.Create(
                        null, null,
                        UnifiedVariableIdentifier.Create(namespaceName));
                ...
                try {
                    ...
                    case ".java":
                        ...
                        new JavaProgramGenerator().GenerateFromFile(filePath).
                                Accept(_visitor);
                        break;
                    case ".js":
                        //_visitor.CodeFactory = new JavaScriptCodeFactory();
                        _visitor.LanguageValue = 2;
                        new JavaScriptProgramGenerator().GenerateFromFile(
                                filePath).Accept(_visitor);
                        break;
                    case ".cs":
                        //_visitor.CodeFactory = new CSharpCodeFactory();
                        _visitor.LanguageValue = 1;
                        new CSharpProgramGenerator().GenerateFromFile(filePath).
                                Accept(_visitor);
                        break;
                    case ".rb":
                        //_visitor.CodeFactory = new Ruby18CodeFactory();
                        _visitor.LanguageValue = 4;
                        new Ruby18ProgramGenerator().GenerateFromFile(filePath).
                                Accept(_visitor);
                        break;
                    case ".py":
                        //_visitor.CodeFactory = new Python2CodeFactory();
                        _visitor.LanguageValue = 3;
                        new Python2ProgramGenerator().GenerateFromFile(filePath)
                                .Accept(_visitor);
                        break;
                    }
                } catch {}
